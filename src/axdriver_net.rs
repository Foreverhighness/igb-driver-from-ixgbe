...
use crate::{IgbDevice, IgbError, IgbHal, IgbNetBuf, MemPool, NicDevice};
...
fn ixgbe_ptr_to_buf(ptr: NetBufPtr, pool: &Arc<MemPool>) -> DevResult<IgbNetBuf> {
    IgbNetBuf::construct(ptr.raw_ptr::<u8>() as usize, pool, ptr.packet_len())
        .map_err(|_| DevError::BadState)
}
